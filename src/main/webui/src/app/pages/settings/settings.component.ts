...
export class SettingsComponent {
  ...
  /** On macOS there is no overlay at all (NoOpOverlayWindow); the entire tab is disabled. */
  readonly overlayAvailable = computed(() => this.platform.os() !== 'mac');

  /**
   * Overlay styling (position, size, colours) only applies on Windows, where the app draws its own
   * overlay. On Linux the desktop's native OSD owns appearance; on macOS there is no overlay at all.
   * Enable / Show number / Scale still apply on Linux. Defaults to supported until the backend
   * platform answers, so the controls aren't briefly greyed out on load.
   */
  readonly overlayStylingSupported = computed(() => this.platform.os() === 'windows');
